...
Module Commands
    ...
    Public Sub Exec(RL As String)
        If RL.Equals("help") Then
            ...
            Console.WriteLine("save <s> <c>|clears | Save a command <c> on a slot <s> or clear (clears) the slot.")
            Console.WriteLine("autostart <c>|clears | Save a command <c> to do on " & Reference.AppName & "'s startup or clear (clears) the autostart command.")
            Console.WriteLine("echo <t> | Echo text <t>.")
        ...
        ElseIf RL.StartsWith("slot ") Then
            ...
            If RL = "1" Then
                Exec(My.Settings.slot1)
            ElseIf RL = "2" Then
                Exec(My.Settings.slot2)
            ElseIf RL = "3" Then
                Exec(My.Settings.slot3)
            ElseIf RL = "4" Then
                Exec(My.Settings.slot4)
            ElseIf RL = "5" Then
                Exec(My.Settings.slot5)
            ElseIf RL = "6" Then
                Exec(My.Settings.slot6)
            End If
